...
def standardize_m3(m3_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df[["sku", "lot", "depot", "emplacement", "type", "qty_m3"]]
...
def build_reflex_m3_wide_with_lotless(
    ...
    m3_with_lot_agg = (
        m3_with_lot
        .groupby(["sku", "lot", "category", "depot", "type"], dropna=False)["qty_m3"]
        .sum()
        .reset_index()
    )
    ...
    m3_no_lot_agg = (
        m3_no_lot
        .groupby(["sku", "category", "depot", "type"], dropna=False)["qty_m3"]
        .sum()
        .reset_index()
    )
